...
const formatDate = (dateObj: String) => { // poor implementation, Dayjs is not initialising properly, instead as string
...
const DrawPieChart: React.FC<DrawPieChartProps> = ({ transactions }) => {
  ...
  const groupedItems = Object.values(transactions.reduce((acc: Record<string, any>, item: Transaction, index) => {
      ...
      {transactions.map((transaction, index) => (
        ...
        <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
          ...
          <div style={{ textAlign: 'left', columnWidth: '450px' }}>
            <h3 style={{ margin: '0', color: 'white' }}>{transaction.whatFor}</h3>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                <p>{formatDate(transaction.dateTime)} {"  ·  "} {transaction.whatBrought}</p>
            </div>
          </div>
          ...
          <div style={{ 
              ...
              <div style={{ width: '6px', height: '6px', backgroundColor: '#444ce7', borderRadius: '50%', marginRight: '4px' }}></div>
              {transaction.type}
          </div>

          {/* Placeholder for the $amount */}
          <div style={{ textAlign: 'right', marginLeft: 'auto', color: 'white' }}>
